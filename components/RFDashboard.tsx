...
export const RFDashboard: React.FC<RFDashboardProps> = ({ result }) => {
  ...
  if (!result) {
    return (
      <div className="flex flex-col h-full bg-white border border-gray-200 rounded-lg items-center justify-center text-gray-500 p-8 text-center min-h-[400px]">
        ...
        {activeTab === 'profile' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                <DataField label="Lecture Fee" value={parsedData.lectureFee ? `₱${parsedData.lectureFee.toLocaleString()}` : undefined} />
                <DataField label="Lab Fees" value={parsedData.labFees ? `₱${parsedData.labFees.toLocaleString()}` : undefined} />
                  ...
                  {parsedData.subjects && parsedData.subjects.length > 0 ? (
                    parsedData.subjects.map((subject, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        <td className="px-4 py-3 font-medium text-gray-900"><input type="text" className="bg-transparent w-full border-b border-transparent focus:border-blue-600 focus:outline-none" defaultValue={subject.code} /></td>
                        <td className="px-4 py-3"><input type="text" className="bg-transparent w-full border-b border-transparent focus:border-blue-600 focus:outline-none" defaultValue={subject.title} /></td>
                        <td className="px-4 py-3 text-right"><input type="number" className="bg-transparent w-12 text-right border-b border-transparent focus:border-blue-600 focus:outline-none" defaultValue={subject.lecUnits} /></td>
                        <td className="px-4 py-3 text-right"><input type="number" className="bg-transparent w-12 text-right border-b border-transparent focus:border-blue-600 focus:outline-none" defaultValue={subject.labUnits} /></td>
                        <td className="px-4 py-3 text-right"><input type="number" className="bg-transparent w-12 text-right border-b border-transparent focus:border-blue-600 focus:outline-none" defaultValue={subject.totalCredits} /></td>
                        <td className="px-4 py-3 text-right"><input type="number" className="bg-transparent w-20 text-right border-b border-transparent focus:border-blue-600 focus:outline-none" defaultValue={subject.fee} /></td>
                        <td className="px-4 py-3"><input type="text" className="bg-transparent w-full border-b border-transparent focus:border-blue-600 focus:outline-none text-xs" defaultValue={subject.scheduleAndRoom} /></td>
                      </tr>
                    ))
                  ) : (
                    <tr>
