import { USAEnrollmentForm, Subject } from '@/types/rf';
...
export function parseRFText(text: string): Partial<USAEnrollmentForm> {
  ...
  const lectureFeeMatch = text.match(/(?:Lecture Fee)\s*[:|]?\s*[P|₱]?\s*([\d,.]+)/i);
  if (lectureFeeMatch) {
    data.lectureFee = parseFloat(lectureFeeMatch[1].replace(/,/g, ''));
  }
  const labFeeMatch = text.match(/(?:Lab Fees?)\s*[:|]?\s*[P|₱]?\s*([\d,.]+)/i);
  if (labFeeMatch) {
    data.labFees = parseFloat(labFeeMatch[1].replace(/,/g, ''));
  }
